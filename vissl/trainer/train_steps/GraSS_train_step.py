...
def DACrop(single_sample,headmap,max_ratio):
    """
    DACrop
    """
    max_v=np.max(headmap)
    min_v=np.min(headmap)
    length=max_v-min_v
    
    bin_target=(headmap>min_v+length*max_ratio).astype('uint8')

    bin_target_without_small=morphology.remove_small_objects(bin_target,1,connectivity=2)
    label_image=measure.label(bin_target_without_small)
    regions=measure.regionprops(label_image)

    region_headmap_max=min_v 
    box_target=[0,0,headmap.shape[0],headmap.shape[1]] 
    for region in regions:
        box = region.bbox
        tmp=headmap[box[0]:box[2], box[1]:box[3]] 
        tmp=np.max(tmp)
        
        if tmp>=region_headmap_max:
            region_headmap_max=tmp
            box_target=box
    
    if region_headmap_max==min_v: 
        sample_GraSS=single_sample
    else:
        sample_GraSS=single_sample[:,:,box_target[0]:box_target[2], box_target[1]:box_target[3]].clone() 
        sample_GraSS=F.interpolate(sample_GraSS, size=[headmap.shape[0],headmap.shape[1]], mode='nearest')

    return sample_GraSS


def getHeadMapCut(sample_tmp,max_ratio,loss,features):
    """
    Get LAM
    """
    ...
    SampleWeight=sample.shape[3]

    feature_grad=[]
    def extract(grad):
        feature_grad.append(grad)
    features.register_hook(extract)
    loss.backward() 
    greds_all=feature_grad[0]

    pooled_grads=F.adaptive_avg_pool2d(greds_all,(1,1))
    features*=pooled_grads
    headmap_all=torch.mean(features,axis=1)

    tmp=headmap_all.view(features.shape[0],-1)
    tmp=torch.max(tmp,axis=1)[0].reshape([features.shape[0],1])
    tmp=(tmp.expand([features.shape[0],features.shape[2]])).reshape([features.shape[0],features.shape[2],1])
    tmp=tmp.expand([features.shape[0],features.shape[2],features.shape[3]])
    headmap_all=headmap_all/tmp

    headmap_all=headmap_all.unsqueeze(1)

    for sample_i in range(SampleNum):
        headmap=headmap_all[sample_i,0,...].cpu().detach().numpy()
        headmap = cv2.resize(headmap, (SampleHeight, SampleWeight)) 
    
        sample_cut=DACrop(sample[sample_i:sample_i+1,:,:,:],headmap,max_ratio)  # DACrop
        
        sample[sample_i,:,:,:]=sample_cut 
...
@register_train_step("GraSS_train_step")
def GraSS_train_step(task):
    """
    ...
    task.last_batch = LastBatchInfo() # LastBatchInfo=SimpleNamespace，
    ...
    with grad_context, ddp_context, torch_amp_context:
        ...
        with PerfTimer("forward", perf_stats), record_function("forward"):
            ...
            model_output,features = task.model(sample["input"]) 
    ...
    with PerfTimer('Contrastive Loss Gradient Guide',perf_stats),record_function("Contrastive Loss Gradient Guide"):
